...
class Crud_model extends CI_Model { 
    ...
    function get_students($class_id){
        // Get students from student table for this class
        $query = $this->db->order_by('name', 'ASC')->get_where('student', array('class_id' => $class_id));
        return $query->result_array();
    }
    ...
    function system_logo(){
        ...
        // Save with original extension (TCPDF can handle most formats)
        $logoPath = $uploadDir . 'logo.' . $fileExt;
        
        // Delete all old logo file variants to avoid confusion
        $oldLogoFiles = array(
            $uploadDir . 'logo.jpg',
            $uploadDir . 'logo.jpeg',
            $uploadDir . 'logo.png',
        ...
        // Update settings table with logo path (keep original extension)
        $data['description'] = 'uploads/logo.' . $fileExt;
    ...
    function update_settings(){
        ...
        $data['description']    =   $this->input->post('principal_name');
        $this->db->where('type', 'principal_name');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('principal_title');
        $this->db->where('type', 'principal_title');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('vice_principal_name');
        $this->db->where('type', 'vice_principal_name');
        $this->db->update('settings', $data);
        ...
        $data['description']    =   $this->input->post('board_affiliation');
        $this->db->where('type', 'board_affiliation');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('accreditation_no');
        $this->db->where('type', 'accreditation_no');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('registration_no');
        $this->db->where('type', 'registration_no');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('year_established');
        $this->db->where('type', 'year_established');
        $this->db->update('settings', $data);

        // Feature Toggles
        $data['description']    =   $this->input->post('features_hostel');
        $this->db->where('type', 'features_hostel');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_transportation');
        $this->db->where('type', 'features_transportation');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_clubs');
        $this->db->where('type', 'features_clubs');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_events');
        $this->db->where('type', 'features_events');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_alumni');
        $this->db->where('type', 'features_alumni');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_online_payments');
        $this->db->where('type', 'features_online_payments');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('features_sms');
        $this->db->where('type', 'features_sms');
        $this->db->update('settings', $data);

        // Email Configuration Settings
        $data['description']    =   $this->input->post('email_enabled');
        $this->db->where('type', 'email_enabled');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('email_address');
        $this->db->where('type', 'email_address');
        $this->db->update('settings', $data);
        ...
        $data['description']    =   $this->input->post('email_from_name');
        $this->db->where('type', 'email_from_name');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('email_from_address');
        $this->db->where('type', 'email_from_address');
        $this->db->update('settings', $data);

        $data['description']    =   $this->input->post('email_subject_receipt');
        $this->db->where('type', 'email_subject_receipt');
        $this->db->update('settings', $data);
    }
